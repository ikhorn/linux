...
#define CPSW_HEADROOM (max(XDP_PACKET_HEADROOM, NET_SKB_PAD) + NET_IP_ALIGN)
...
static int cpsw_xdp_rxq_reg(struct cpsw_common *cpsw, int ch)
{
	 ...
	 * can have different memory model an so, network device must hold it's
